...
public class SubunidadOrganizativaServicioImpl implements SubunidadOrganizativaServicio{
	...
	@Override
	public List<SubunidadOrganizativaDTO> findByUnidadOrganizativa(UnidadOrganizativa unidad) {
		List<SubunidadOrganizativa> subunidades = this.subunidadOrganizativaRepositorio
				.findByUnidadOrganizativaId(unidad);
		List<SubunidadOrganizativaDTO> subunidadesDTO = new ArrayList<>();

		for (SubunidadOrganizativa subunidad: subunidades
			 ) {
			 SubunidadOrganizativaDTO subunidadDTO = MHelpers.modelMapper().map(subunidad, SubunidadOrganizativaDTO.class);

			 subunidadesDTO.add(subunidadDTO);
		}
		return subunidadesDTO;
	}
